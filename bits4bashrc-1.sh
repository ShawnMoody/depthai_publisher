...
function run349 (){
    ...
    tmux new-session -s UAVTeam3\; set -g mouse on\; send-keys "roscore" C-m\; split-window -h -p 85\; select-pane -t 0\; split-window -v\;                   \
        ...
        send-keys "sleep 5; roslaunch ${catkin_ws}/launch/control.launch" C-m\; split-window -v -p 75\;               \
        send-keys "sleep 10; roslaunch qutas_lab_450 environment.launch" C-m\; split-window -v -p 60\;                                 \
        send-keys "sleep 5; rosrun depthai_publisher dai_publisher" C-m\; split-window -v \;                                        \
        send-keys "sleep 5; rosrun depthai_publisher aruco_subscriber" \;  select-pane -t 6\;                                   \
